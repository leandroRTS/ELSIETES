...
def processUploadFiles(filename,filesize,files,update,bot,message,thread=None,jdb=None):
    try:
        ...
        if cloudtype == 'moodle':
            ...
            if user_info['tokenize']!=0:
               tokenize = True
            bot.editMessageText(message,'⏫Subiendo Espere Mientras tómese un café ☕ y relájese que puede dar error  😄')
    ...
    try:
        ...
        if '/zips' in msgText:
            ...
            if getUser:
                try:
                   ...
                   jdb.save()
                   msg = 'El tamaño de los zips seran de '+ sizeof_fmt(size*1024*1024)+'📚 las partes👍🏻'
                   bot.sendMessage(update.message.chat.id,msg)
                except:
        ...
        if '/uptype' in msgText:
            try:
                ...
                bot.sendMessage(update.message.chat.id,'❌Error en el comando /uptype (typo de subida (evidence,calendar,draft,blog,perfil))❌')
            return
        if '/proxy' in msgText:
            try:
                cmd = str(msgText).split(' ',2)
        ...
        if '/start' in msgText:
            ...
            #    bot_token = '5163560775:AAHy2x_8tr8hepLAlNvrbMzF2PkR_pPK0NY'
...
def main():
    ...
    #set in debug
    bot_token = '5596412627:AAE-U3OTLMTJjgD_viC2wDFiG0x1QZXhstQ'
